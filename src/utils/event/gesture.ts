...
class TwoFingerOperate implements Operate {
  ...
  update(positions: Vector[]) {
    ...
    this.ViewObject.handleChangeScaleByTouch(scale,angle);
    this.oldDist = dist;
  }
}
...
class Gesture {
  ...
  private longpressEventList: Array<listenCallback> =
    new Array<listenCallback>();
  private twoTouchEventList: Array<listenCallback> =
    new Array<listenCallback>();
   ...
   */
  public addListenGesti(
    gestiType: "click" | "longpress" | "dbclick" | "twotouch" | "globalClick",
    listenCallback: listenCallback | GlobalListenCallback
  ) {
    if (gestiType === "click")
      this.clickEventList.push(listenCallback as listenCallback);
    if (gestiType === "longpress")
      this.longpressEventList.push(listenCallback as listenCallback);
    if (gestiType === "dbclick")
      this.dbclickEventList.push(listenCallback as listenCallback);
    if (gestiType === "twotouch")
      this.twoTouchEventList.push(listenCallback as listenCallback);
    if (gestiType === "globalClick")
      this.globalClickEventList.push(listenCallback as GlobalListenCallback);
  }
  public cancel(): void {
  ...
  update(positions: Vector[]) {
    if (this.operate == null || this.disabled) return;
